package ru.relicarium.pledge.application.service.impl;

import jakarta.persistence.EntityNotFoundException;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import ru.relicarium.pledge.application.dto.RedeemPledgeCommand;
import ru.relicarium.pledge.application.service.PledgeRedemptionService;
import ru.relicarium.pledge.application.service.PledgeStatusService;
import ru.relicarium.pledge.domain.enums.PledgeStatus;
import ru.relicarium.pledge.domain.enums.PledgeStatusTransition;
import ru.relicarium.pledge.domain.model.Pledge;
import ru.relicarium.pledge.integration.LedgerClient;
import ru.relicarium.pledge.integration.dto.LedgerLoanResponse;
import ru.relicarium.pledge.integration.dto.LedgerRepayRequest;
import ru.relicarium.pledge.persistence.repository.PledgeRepository;

import java.time.OffsetDateTime;
import java.util.UUID;

@Service
@RequiredArgsConstructor
public class PledgeRedemptionServiceImpl implements PledgeRedemptionService {

    private final PledgeRepository pledgeRepository;
    private final LedgerClient ledgerClient;
    private final PledgeStatusService pledgeStatusService;


    @Override
    @Transactional
    public Pledge redeem(UUID pledgeId, RedeemPledgeCommand command) {

        Pledge pledge = pledgeRepository.findById(pledgeId).orElseThrow(() ->
                new EntityNotFoundException("Pledge not found " + pledgeId));

        PledgeStatus pledgeStatus = pledge.getStatus();

        if (pledgeStatus != PledgeStatus.ACTIVE && pledgeStatus != PledgeStatus.GRACE) {
            throw new IllegalStateException("Redemption allowed only for ACTIVE or GRACE");
        }
        LedgerRepayRequest ledgerRepayRequest = new LedgerRepayRequest(
                pledge.getId(),
                command.amount(),
                command.operationId(),
                command.paidAt() != null ? command.paidAt() : OffsetDateTime.now()
        );
        LedgerLoanResponse ledgerLoanResponse = ledgerClient.repay(ledgerRepayRequest);

        if (ledgerLoanResponse.closedAt() == null) {
            throw new IllegalStateException("Loan not closed after repayment; redemption requires full payoff");
        }
        pledge = pledgeStatusService.changeStatus(pledge.getId(), PledgeStatusTransition.REDEEM);

        return pledge;
    }
}
